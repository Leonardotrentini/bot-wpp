...
import { History, Receipt, ShoppingBag, Loader2 } from 'lucide-react'
...
export function ContactLeadActions({ contact, conversationId, onContactUpdate, onConversationUpdate }) {
  ...
  const handleSaveQuote = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Falha ao salvar orçamento.')
    } finally {
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Falha ao confirmar compra.')
    } finally {
  ...
  const btnClass =
    'flex flex-1 flex-col items-center gap-1 rounded-xl border border-accent-500/35 bg-accent-500/10 px-2 py-2.5 text-[10px] font-semibold uppercase tracking-wide text-accent-400 transition hover:border-accent-500/55 hover:bg-accent-500/20 hover:text-accent-300'

  return (
    <>
      <div>
        <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-stone-500">Lead</p>
        {(contact.quote || contact.purchase) && (
